...
fn cmd_srem_sismember() {
    ...
    assert_int_eq(
        &run_cmd(&db, &se, &mut idx, &["SISMEMBER", "s", "a"]),
        0,
    );
    assert_int_eq(
        &run_cmd(&db, &se, &mut idx, &["SISMEMBER", "s", "b"]),
        1,
    );
}
...
fn cmd_sunion_sinter_sdiff() {
    ...
    let mut u = frame_array_all_bulk(run_cmd(
        &db,
        &se,
        &mut idx,
        &["SUNION", "s1", "s2"],
    ...
    let mut i = frame_array_all_bulk(run_cmd(
        &db,
        &se,
        &mut idx,
        &["SINTER", "s1", "s2"],
    ...
    i.sort();
    assert_eq!(i, vec!["b"]);
    let d = frame_array_all_bulk(run_cmd(
        &db,
        &se,
        &mut idx,
        &["SDIFF", "s1", "s2"],
...
fn cmd_sunionstore() {
    ...
    let mut m = frame_array_all_bulk(run_cmd(
        &db,
        &se,
        &mut idx,
        &["SMEMBERS", "dst"],
...
fn cmd_smove() {
    ...
    assert_int_eq(
        &run_cmd(&db, &se, &mut idx, &["SISMEMBER", "src", "m"]),
        0,
    );
    assert_int_eq(
        &run_cmd(&db, &se, &mut idx, &["SISMEMBER", "dst", "m"]),
        1,
    );
}
